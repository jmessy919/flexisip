...
public:
	virtual ~AgentConfigurator() = default;
	virtual void configureAgent(GenericManager& cfg) = 0;
};

class RegisterTestAgentConfiguratorBase : public AgentConfigurator {
public:
	void configureAgent(GenericManager& cfg) override {
		auto* globalCfg = cfg.getRoot()->get<GenericStruct>("global");
		globalCfg->get<ConfigValue>("transports")->set("sip:*:5160");
		globalCfg->get<ConfigValue>("aliases")->set("127.0.0.1");

		auto* dosProtectionCfg = cfg.getRoot()->get<GenericStruct>("module::DoSProtection");
		dosProtectionCfg->get<ConfigValue>("enabled")->set("false");
...
class InternalDbConfig : public RegisterTestAgentConfiguratorBase {
public:
	void configureAgent(GenericManager& cfg) override {
		RegisterTestAgentConfiguratorBase::configureAgent(cfg);
		auto* registrarCfg = cfg.getRoot()->get<GenericStruct>("module::Registrar");
		registrarCfg->get<ConfigValue>("db-implementation")->set("internal");
	}
};

class RedisDbConfig : public RegisterTestAgentConfiguratorBase {
public:
	void configureAgent(GenericManager& cfg) override {
		RegisterTestAgentConfiguratorBase::configureAgent(cfg);
		...
		auto* registrarCfg = cfg.getRoot()->get<GenericStruct>("module::Registrar");
		registrarCfg->get<ConfigValue>("db-implementation")->set("redis");
		registrarCfg->get<ConfigValue>("redis-server-domain")->set("127.0.0.1");
		registrarCfg->get<ConfigValue>("redis-server-port")->set(to_string(redisPort));
	}
...
class DuplicatePushTokenRegisterBase : public AgentTest {
public:
	class RegisterBindListener : public ContactUpdateListener {
	public:
		RegisterBindListener(DuplicatePushTokenRegisterBase& test) : mTest{test} {
		}
	...
	private:
		DuplicatePushTokenRegisterBase& mTest;
	};

	class RegisterFetchListener : public ContactUpdateListener {
	public:
		RegisterFetchListener(DuplicatePushTokenRegisterBase& test,
							  int expectedNumberOfContact,
							  const string& mustBePresentUuid = "")
			: mTest{test}, mExpectedNumberOfContact{expectedNumberOfContact}, mMustBePresentUuid{mustBePresentUuid} {};
	...
	template <typename T>
	DuplicatePushTokenRegisterBase(T&& config) : mConfig{std::forward<T>(config)} {
	}
...
private:
	...
	void onExec() override {
		...
		// APNS (simple ones)
		insertContact("sip:apns1@sip.example.org",
		...
		insertContact("sip:apns9@sip.example.org", "pn-provider=apns;pn-prid=aRemoteToken:remote&aPushKitToken:voip;pn-"
												   "param=aProjectID.aBundleID.remote&voip");
		insertContact("sip:apns10@sip.example.org",
		...
		insertContact("sip:apns12@sip.example.org",
					  "pn-provider=apns;pn-prid=aRemoteToken:remote&aPushKitToken:voip;pn-"
					  "param=aProjectID.aBundleID.remote&voip");
		insertContact("sip:apns13@sip.example.org",
					  "pn-provider=apns;pn-prid=aRemoteToken:remote&aPushKitToken:voip;pn-"
					  "param=aProjectID.aBundleID.remote&voip");

		// Multiple entries with same tokens are possible with Redis because no cleaning is done on biding, except if
		// unique key are the same.
		...
		insertContact("sip:apns14@sip.example.org",
		...
		// FCM
		sendRegisterRequest("sip:fcm1@sip.example.org", "pn-provider=fcm;pn-prid=aFcmToken;pn-param=aProjectId",
							"fcm1Reg");
		...
		sendRegisterRequest("sip:fcm3@sip.example.org",
							"pn-provider=fcm;pn-prid=aOtherUniqueFcmToken;pn-param=aProjectId", "fcm3Reg");
		sendRegisterRequest("sip:fcm4@sip.example.org",
		...
		checkResultInDb(SipUri{"sip:fcm1@sip.example.org"}, make_shared<RegisterFetchListener>(*this, 1, "fcm1Reg"),
						true);
		// Different prid and param --> both kept
		checkResultInDb(SipUri{"sip:fcm2@sip.example.org"}, make_shared<RegisterFetchListener>(*this, 2), true);
		// Different prid but same param --> both kept
		checkResultInDb(SipUri{"sip:fcm3@sip.example.org"}, make_shared<RegisterFetchListener>(*this, 2), true);
		// Same prid but different param --> both kept
		checkResultInDb(SipUri{"sip:fcm4@sip.example.org"}, make_shared<RegisterFetchListener>(*this, 2), true);

		// APNS (simple ones)
		// Same prid and param --> replaced
		checkResultInDb(SipUri{"sip:apns1@sip.example.org"}, make_shared<RegisterFetchListener>(*this, 1, "apns1Reg"),
						true);
		// Different prid but same param --> both kept
		checkResultInDb(SipUri{"sip:apns2@sip.example.org"}, make_shared<RegisterFetchListener>(*this, 2), true);
		// Same prid but different param --> both kept
		checkResultInDb(SipUri{"sip:apns3@sip.example.org"}, make_shared<RegisterFetchListener>(*this, 2), true);

		// APNS (with 2 tokens)
		// All same (only param suffix is reversed) --> replaced
		checkResultInDb(SipUri{"sip:apns4@sip.example.org"}, make_shared<RegisterFetchListener>(*this, 1, "apns4Reg"),
						true);
		// Same PushKitToken, different RemoteToken, same param --> replaced
		checkResultInDb(SipUri{"sip:apns5@sip.example.org"}, make_shared<RegisterFetchListener>(*this, 1, "apns5Reg"),
						true);
		// Different PushKitToken, same RemoteToken, same param --> replaced
		checkResultInDb(SipUri{"sip:apns6@sip.example.org"}, make_shared<RegisterFetchListener>(*this, 1, "apns6Reg"),
						true);
		// Same PushKitToken, same RemoteToken, Different param --> both kept
		checkResultInDb(SipUri{"sip:apns7@sip.example.org"}, make_shared<RegisterFetchListener>(*this, 2), true);
		// Badly formated register, can't really compare --> both kept
		checkResultInDb(SipUri{"sip:apns8@sip.example.org"}, make_shared<RegisterFetchListener>(*this, 2), true);
		// Invalid prid ('&' not present), can't really compare --> both kept
		checkResultInDb(SipUri{"sip:apns9@sip.example.org"}, make_shared<RegisterFetchListener>(*this, 2), true);
		// Invalid prid (only remote present), can't really compare --> both kept
		checkResultInDb(SipUri{"sip:apns10@sip.example.org"}, make_shared<RegisterFetchListener>(*this, 2), true);
		// Invalid prid (only suffix for remote and voip), can't really compare --> both kept
		checkResultInDb(SipUri{"sip:apns11@sip.example.org"}, make_shared<RegisterFetchListener>(*this, 2), true);
		// Invalid pn-param ('_' before instead of '.'), can't really compare --> both kept
		checkResultInDb(SipUri{"sip:apns12@sip.example.org"}, make_shared<RegisterFetchListener>(*this, 2), true);
		// Invalid pn-param (no '.'), can't really compare --> both kept
		checkResultInDb(SipUri{"sip:apns13@sip.example.org"}, make_shared<RegisterFetchListener>(*this, 2), true);
		...
		checkResultInDb(SipUri{"sip:elisa@sip.example.org"}, make_shared<RegisterFetchListener>(*this, 1, "elisa15"),
						true);

		// Legacy contact parameters (apple)
		// Same prid and param --> replaced
		checkResultInDb(SipUri{"sip:apns14@sip.example.org"}, make_shared<RegisterFetchListener>(*this, 1, "apns14Reg"),
						true);
		// Same prid and param --> replaced
		checkResultInDb(SipUri{"sip:apns15@sip.example.org"}, make_shared<RegisterFetchListener>(*this, 1, "apns15Reg"),
						true);
		// Different prid but same param --> both kept
		checkResultInDb(SipUri{"sip:apns16@sip.example.org"}, make_shared<RegisterFetchListener>(*this, 2), true);

		// Legacy contact parameters (firebase)
		// Same prid and param --> replaced
		checkResultInDb(SipUri{"sip:fcm5@sip.example.org"}, make_shared<RegisterFetchListener>(*this, 1, "fcm5Reg"),
						true);
		// Different prid but same param --> both kept
		checkResultInDb(SipUri{"sip:fcm6@sip.example.org"}, make_shared<RegisterFetchListener>(*this, 2), true);
	}

	/**
	 * Insert a contact into the registrarDB.
	 */
	void insertContact(const string& sipUri, const string& paramList) {
		sofiasip::Home home{};
		SipUri user{sipUri + ";" + paramList};
		BindingParameters parameter{};
		parameter.globalExpire = 1000;
		parameter.callId = "random_id_necessary_to_bind_" + to_string(mNotSoRandomId++);
		parameter.withGruu = true;

		auto contact = sip_contact_create(home.home(), (url_string_t*)user.str().c_str(), nullptr);
		RegistrarDb::get()->bind(user, contact, parameter, make_shared<RegisterBindListener>(*this));
		mExpectedBindingDone++;
		waitFor([this]() { return mBindingDone == mExpectedBindingDone; }, 2s);
	}

	/**
	 * Send REGISTER requests
	 */
	void sendRegisterRequest(const string& sipUri, const string& paramList, const string& uuid) {

		BellesipUtils bellesipUtils{"0.0.0.0", -1, "UDP", [this](int status) {
										if (status != 100) {
		...
		bellesipUtils.sendRawRequest(
			...
			"To: <" + sipUri + ">\r\n"
			"Call-ID: 1053183492" + to_string(mNotSoRandomId++)+"\r\n"
			"CSeq: 1 REGISTER\r\n"
		...
		mExpectedResponseReceived++;
		auto beforePlus2 = system_clock::now() + 2s;
		while (mResponseReceived != mExpectedResponseReceived && beforePlus2 >= system_clock::now()) {
			mAgent->getRoot()->step(20ms);
			bellesipUtils.stackSleep(20);
		}
	}

	void checkResultInDb(SipUri uri, shared_ptr<RegisterFetchListener> fetchListener, bool recursive) {
		RegistrarDb::get()->fetch(uri, fetchListener, recursive);
		mExpectedFetchingDone++;
		waitFor([this]() { return mFetchingDone == mExpectedFetchingDone; }, 2s);
	}

	// Private attributes
	std::unique_ptr<AgentConfigurator> mConfig{};
	int mResponseReceived{0};
	int mExpectedResponseReceived{0};
	int mNotSoRandomId{0};
	int mBindingDone{0};
	int mExpectedBindingDone{0};
	int mFetchingDone{0};
	int mExpectedFetchingDone{0};

	friend class RegisterFetchListener;
};

template <typename ConfigT>
class DuplicatePushTokenRegister : public DuplicatePushTokenRegisterBase {
public:
	DuplicatePushTokenRegister() : DuplicatePushTokenRegisterBase{make_unique<ConfigT>()} {
	}
};

static test_t tests[] = {
    TEST_NO_TAG("Duplicate push token at register handling, with internal db",
                run<DuplicatePushTokenRegister<InternalDbConfig>>),
    TEST_NO_TAG("Duplicate push token at register handling, with Redis db",
                run<DuplicatePushTokenRegister<RedisDbConfig>>),
};

test_suite_t registerSuite = {"Register", nullptr, nullptr, nullptr, nullptr, sizeof(tests) / sizeof(tests[0]), tests};
