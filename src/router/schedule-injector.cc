...
void ScheduleInjector::injectRequestEvent(const std::shared_ptr<RequestSipEvent>& ev,
	...
	if (contactMapEntry == injectMap.end()) {
		...
		mRouter->getAgent()->injectRequestEvent(ev);
		return;
	}
...
void ScheduleInjector::startInject(sofiasip::MsgSipPriority currentWorkingPriority,
	...
	while (it != contactInjectContexts.end() && it->waitForInject) {
		mRouter->getAgent()->injectRequestEvent(it->waitForInject);
...
void ScheduleInjector::continueInjectIfNeeded(sofiasip::MsgSipPriority currentWorkingPriority,
	...
	} else if (currentWorkingPriority == MsgSipPriority::Normal) {
		continueInjectIfNeeded(MsgSipPriority::NonUrgent, contactId);
	}
}
...
void ScheduleInjector::removeContext(const shared_ptr<ForkContext>& fork, const string& contactId) {
	auto& injectMap = getMapFromPriority(fork->getMsgPriority());
	...
	if (reverseIt != contactInjectContexts.rend()) {
		contactInjectContexts.erase(std::next(reverseIt).base());
		if (contactInjectContexts.empty()) {
...
bool ScheduleInjector::areAllHigherPriorityMapEmpty(sofiasip::MsgSipPriority msgSipPriority,
	...
	if (msgSipPriority == MsgSipPriority::Emergency) {
		return true;
	} else if (msgSipPriority == MsgSipPriority::Urgent) {
		return mEmergencyInjectContexts.find(contactId) == mEmergencyInjectContexts.end();
	} else if (msgSipPriority == MsgSipPriority::Normal) {
		return mUrgentInjectContexts.find(contactId) == mUrgentInjectContexts.end();
	} else { // MsgSipPriority::NonUrgent
		return mUrgentInjectContexts.find(contactId) == mUrgentInjectContexts.end() &&
		       mNormalInjectContexts.find(contactId) == mNormalInjectContexts.end();
	}
}
