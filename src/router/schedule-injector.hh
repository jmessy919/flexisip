 ...
 *
 * Every calls to Agent::injectRequestEvent(RequestSipEvent ev) will be done in same order than calls to
...
public:
	explicit ScheduleInjector(ModuleRouter* router) : Injector(router){};
	...
	/**
	 * Create the order that will be enforce by ScheduleInjector::injectRequestEvent.
	 */
