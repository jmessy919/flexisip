...
nta_agent_t* Module::getSofiaAgent() const {
	return mAgent.lock()->getSofiaAgent();
}
...
void ModuleToolbox::cleanAndPrependRoute(AgentInternalInterface* ag, msg_t* msg, sip_t* sip, sip_route_t* r) {
...
void ModuleToolbox::addRecordRoute(AgentInternalInterface* ag,
...
void ModuleToolbox::addRecordRouteIncoming(AgentInternalInterface* ag, const shared_ptr<RequestSipEvent>& ev) {
...
bool ModuleToolbox::isManagedDomain(const AgentInternalInterface* agent,
...
void ModuleToolbox::addPathHeader(AgentInternalInterface* ag, const shared_ptr<RequestSipEvent>& ev, tport_t* tport, const char* uniq) {
	su_home_t* home = ev->getMsgSip()->getHome();
	msg_t* msg = ev->getMsgSip()->getMsg();
	sip_t* sip = ev->getMsgSip()->getSip();
	url_t* url;
...
const url_t* ModuleToolbox::getNextHop(AgentInternalInterface* ag, const sip_t* sip, bool* isRoute) {
