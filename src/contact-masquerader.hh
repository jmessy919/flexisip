...
public:
	template <typename AgentPtrT, typename StrT>
	ContactMasquerader(AgentPtrT&& agent, StrT&& paramName)
	    : mAgent(std::forward<AgentPtrT>(agent)), mCtRtParamName(std::forward<StrT>(paramName)) {
	}
