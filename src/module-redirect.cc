...
private:
	...
	bool isValidNextConfig(const ConfigValue& cv) final {
		...
		if (cv.getName() == "contact") {
			sip_contact_t* contact = sip_contact_make(mHome.home(), cv.getName().c_str());
			if (!contact) {
	...
	void onLoad(const GenericStruct* mc) final {
		mContact = sip_contact_make(mHome.home(), mc->get<ConfigString>("contact")->read().c_str());
	...
	void onRequest(shared_ptr<RequestSipEvent>& ev) final {
		ev->reply(SIP_302_MOVED_TEMPORARILY, SIPTAG_CONTACT(sip_contact_dup(mHome.home(), mContact)),
		          SIPTAG_SERVER_STR(getAgent()->getServerString()), TAG_END());
	}
	void onResponse(std::shared_ptr<ResponseSipEvent>& ev) final {};
