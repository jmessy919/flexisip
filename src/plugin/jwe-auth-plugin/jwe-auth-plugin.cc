...
static constexpr auto makeArray(T&&... values)
	...
	return array<typename decay<typename common_type<T...>::type>::type, sizeof...(T)>{{forward<T>(values)...}};
}
...
static list<string> listFiles(const string& path, const string& suffix) {
	...
	for (;;) {
		...
		if (file != "." && file != ".." && endsWith(file, dotSuffix)) files.push_back(move(file));
	}
...
void JweAuth::insertJweContext(string&& jweKey, const shared_ptr<JweContext>& jweContext, int timeout) {
	auto* timer = su_timer_create(mAgent.lock()->getRoot()->getTask(), 0);
...
void JweAuth::removeJweContext(su_root_magic_t*, su_timer_t* timer, su_timer_arg_t* arg) {
	auto* jweContext = static_cast<JweContext*>(arg);
