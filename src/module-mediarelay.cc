...
bool MediaRelay::processNewInvite(const shared_ptr<RelayedCall>& c,
	...
	if (sip->sip_from == nullptr || sip->sip_from->a_tag == nullptr) {
		LOGW("No tag in from !");
		return false;
	}
	c->updateActivity();
	...
	string to_tag;
	if (sip->sip_to->a_tag != nullptr) to_tag = sip->sip_to->a_tag;
	string dest_host;
	...
	while (route != nullptr && mAgent.lock()->isUs(route->r_url)) {
		route = route->r_next;
	}
	if (route) {
		dest_host = urlGetHost(route->r_url);
	} else if (sip->sip_request != nullptr && sip->sip_request->rq_url->url_host != nullptr) {
...
void MediaRelay::onRequest(shared_ptr<RequestSipEvent>& ev) {
	...
	if (isInviteOrUpdate(sip->sip_request->rq_method)) {
		...
		if (c == nullptr) c = dynamic_pointer_cast<RelayedCall>(mCalls->find(getAgent().get(), sip, false));
		if (c == nullptr) {
		...
		if ((c = dynamic_pointer_cast<RelayedCall>(mCalls->findEstablishedDialog(getAgent().get(), sip))) != nullptr) {
			mCalls->remove(c);
		}
		 ...
		 * immediately*/
		if (it && (c = it->getProperty<RelayedCall>(getModuleName())) != nullptr) {
...
void MediaRelay::processResponseWithSDP(const shared_ptr<RelayedCall>& c,
	...
	if (sip->sip_to == nullptr || sip->sip_to->a_tag == nullptr) {
		LOGW("No tag in answer");
		return;
	}

	if (sip->sip_status->st_status == 200) {
		if (!c->isDialogEstablished()) c->establishDialogWith200Ok(getAgent().get(), sip);
	...
	string to_tag;
	if (sip->sip_to != nullptr && sip->sip_to->a_tag != nullptr) to_tag = sip->sip_to->a_tag;
...
void MediaRelay::onResponse(shared_ptr<ResponseSipEvent>& ev) {
	...
	if (ot == nullptr && it == nullptr && sip->sip_cseq && isInviteOrUpdate(sip->sip_cseq->cs_method) &&
		...
		shared_ptr<CallContextBase> ccb = mCalls->findEstablishedDialog(getAgent().get(), sip);
		if (ccb) {
			 ...
			 */
			ev->terminateProcessing();
		} else if (mByeOrphanDialogs && mCalls->find(getAgent().get(), sip, true) != nullptr) {
