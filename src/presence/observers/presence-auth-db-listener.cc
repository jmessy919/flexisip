...
flexisip::PresenceAuthDbListener::PresenceAuthDbListener(belle_sip_main_loop_t* mainLoop,
                                                         const shared_ptr<PresentityPresenceInformation>& info)
...
void PresenceAuthDbListener::processResponse(AuthDbResult result, const string& user) {
	...
	if (result == AuthDbResult::PASSWORD_FOUND) {
		...
		public:
			...
			void onError() override{/* Do nothing */};
			void onInvalid() override{/* Do nothing */};
